...
each new OpenCode session to ensure you have the latest changes from the remote
repository.
...
OpenCode uses agent profiles with different permission levels:

- **bootstrap**: Full access for setup and system operations
- **thinking-partner**: Read-only for exploration
- **research-assistant**: Read/write with web access
- **assistant**: Standard permissions with safety checks
- **read-only**: Minimal permissions for safe review

### Commands

Commands are located in `.opencode/command/` and use frontmatter to specify
which agent profile to use. Run commands with `/command-name` in OpenCode.

### Configuration

OpenCode configuration is in `opencode.jsonc` with:
- Agent profiles and permissions
- MCP server configuration
- Plugin system for session hooks
