---
agent: bootstrap
description: Install claudesidian shell command to launch OpenCode from anywhere
...
- Check which shell user is using (bash/zsh/fish)
- Find out current working directory (vault path)
...
- Changes to vault directory: `cd /path/to/your/vault`
- Tries to resume existing session: `opencode --resume 2>/dev/null`
- Falls back to new session if no existing one: `|| opencode`
- All in one command with properly escaped path:
  `(cd "/path/to/vault" && (opencode --resume 2>/dev/null || opencode))`
...
Add to alias to appropriate config file:
...
- Show added line
- Remind user to reload their shell or source to config
- Provide test command

## Shell Detection

Detects user's default shell, with support for command-line override:
...
3. **Escape path**: Properly escape quotes and special characters for shell
   safety
   ```bash
...
4. **Check if already installed**: Search config file for existing
   ...
   if grep -q "alias claudesidian\|function claudesidian" "$CONFIG_FILE"; then
     ...
     grep -A3 "claudesidian" "$CONFIG_FILE"
     echo ""
...
6. **Create backup**: Only if proceeding with modification
   ...
   ```
7. **Build safe alias/function command**: Use escaped path from step3
   ```bash
   # CRITICAL: Use $ESCAPED_PATH in command (not raw $VAULT_PATH)
   if [ "$SHELL_TYPE" = "fish" ]; then
     # Fish uses function syntax, not alias
     COMMAND_TEXT="function claudesidian
     cd \"$ESCAPED_PATH\" && (opencode --resume 2>/dev/null; or opencode)
     cd -
   end"
   else
     # Bash/Zsh use alias syntax
     # IMPORTANT: Use double quotes around $ESCAPED_PATH to preserve escaping
     COMMAND_TEXT="alias claudesidian='(cd \"$ESCAPED_PATH\" && (opencode --resume 2>/dev/null || opencode))'"
   fi
   ```
8. **Remove old command if replacing**:
   ...
   ```
9. **Add command to config file**: Append using escaped command text
   ```bash
...
✅ Installed! Added to /home/user/.zshrc:
   alias claudesidian='(cd "/home/user/My Obsidian Vault" && (opencode --resume 2>/dev/null || opencode))'
...
✅ Installed! Added to /home/user/.config/fish/config.fish:
   function claudesidian
     cd "/home/user/My Obsidian Vault" && (opencode --resume 2>/dev/null; or opencode)
     cd -
   end
...
- **Only claudesidian command is modified** - nothing else in your config
...
2. `opencode --resume 2>/dev/null` - Tries to resume existing session, suppresses
   error
3. `|| opencode` - If resume fails (no session), starts new session
4. After OpenCode exits, subshell closes and returns to original directory
   automatically
...
```fish
function claudesidian
    cd "/path/to/vault" && (opencode --resume 2>/dev/null; or opencode)
    cd -
end
```
...
2. `opencode --resume 2>/dev/null` - Tries to resume existing session, suppresses
   error
3. `; or opencode` - If resume fails (no session), starts new session (Fish
   syntax)
4. `cd -` - Returns to previous directory after OpenCode exits
